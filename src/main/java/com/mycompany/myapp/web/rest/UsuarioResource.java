...
public class UsuarioResource {
    ...
    public List<Usuario> getAllUsuarios(@RequestParam(required = false) String filter) {
        if ("permissao-is-null".equals(filter)) {
            log.debug("REST request to get all Usuarios where permissao is null");
            return StreamSupport
                .stream(usuarioRepository.findAll().spliterator(), false)
                .filter(usuario -> usuario.getPermissao() == null)
                .collect(Collectors.toList());
        }
        if ("pessoa-is-null".equals(filter)) {
            log.debug("REST request to get all Usuarios where pessoa is null");
            return StreamSupport
                .stream(usuarioRepository.findAll().spliterator(), false)
                .filter(usuario -> usuario.getPessoa() == null)
                .collect(Collectors.toList());
        }
